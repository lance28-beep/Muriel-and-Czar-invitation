            ...
            <div className="relative backdrop-blur-sm rounded-xl sm:rounded-2xl overflow-hidden border-2 border-[#9B4719]/30 shadow-xl hover:shadow-2xl transition-all duration-300 hover:border-[#9B4719]/50" style={{ backgroundColor: '#EFD2AA' }}>
              ...
              <div className="relative p-4 sm:p-6 md:p-8 lg:p-10">
                <div className="flex items-center justify-center gap-2.5 sm:gap-3 md:gap-4 mb-3 sm:mb-4 md:mb-5">
                  <div className="bg-[#6A1F08] p-1.5 sm:p-2 md:p-2.5 rounded-full shadow-lg">
                    <Heart className="text-[#EFD2AA] h-3.5 w-3.5 sm:h-4 sm:w-4 md:h-5 md:w-5" fill="#EFD2AA" />
                  </div>
                  <div className="flex flex-col items-center">
                    <h3 className="text-base sm:text-lg md:text-xl lg:text-2xl font-[family-name:var(--font-crimson)] font-semibold text-[#6A1F08]">
                      {totalGuests} {totalGuests === 1 ? "Guest" : "Guests"}{" "}
                      Celebrating With Us
                    </h3>
                    <p className="text-[10px] sm:text-xs md:text-sm text-[#6A1F08]/70 font-[family-name:var(--font-crimson)] mt-0.5 sm:mt-1">
                      {guests.length}{" "}
                      {guests.length === 1 ? "RSVP entry" : "RSVP entries"}
                    </p>
                  </div>
                </div>
                <p className="text-xs sm:text-sm md:text-base text-[#6A1F08]/80 font-[family-name:var(--font-crimson)] leading-relaxed">
              ...
              {isLoading ? (
                <div className="flex items-center justify-center py-16 sm:py-20 md:py-24">
                  <div className="flex flex-col items-center gap-3 sm:gap-4">
                    <Loader2 className="h-10 w-10 sm:h-12 sm:w-12 animate-spin text-[#6A1F08]" />
                    <span className="text-[#6A1F08] font-[family-name:var(--font-crimson)] text-sm sm:text-base md:text-lg">
                      Loading guests...
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-center py-16 sm:py-20 md:py-24">
                  <div className="text-center">
                    <div className="bg-[#6A1F08] w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4">
                      <Heart className="h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8 text-[#EFD2AA]" fill="#EFD2AA" />
                    </div>
                    <h3 className="text-lg sm:text-xl md:text-2xl font-[family-name:var(--font-crimson)] font-semibold text-[#6A1F08] mb-2">
                      No guests have RSVP'd yet
                    </h3>
                    <p className="text-xs sm:text-sm md:text-base text-[#6A1F08]/70 font-[family-name:var(--font-crimson)] max-w-md mx-auto leading-relaxed">
                ...
                <div className="mb-3 sm:mb-5 md:mb-7 lg:mb-9">
                  <div className="space-y-2.5 sm:space-y-3 md:space-y-4">
                    {guests.map((guest, index) => (
                      <div
                        key={index}
                        className="group relative rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-5 border border-[#6A1F08]/20 hover:border-[#6A1F08]/40 transition-all duration-300 hover:shadow-lg"
                        style={{ backgroundColor: '#EFD2AA' }}
                      >
                      <div className="flex flex-col sm:flex-row gap-2.5 sm:gap-3 md:gap-4">
                        {/* Avatar */}
                        <div className="relative h-9 w-9 sm:h-11 sm:w-11 md:h-12 md:w-12 flex-shrink-0">
                          <div className="h-full w-full rounded-full bg-[#6A1F08] text-[#EFD2AA] flex items-center justify-center font-[family-name:var(--font-crimson)] font-semibold shadow-md ring-2 ring-white text-xs sm:text-sm md:text-base">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-3">
                            <div className="flex-1 pr-12 sm:pr-0">
                              <h4 className="font-[family-name:var(--font-crimson)] text-sm sm:text-base md:text-lg font-semibold text-[#6A1F08] mb-0.5 sm:mb-1 group-hover:text-[#6A1F08]/80 transition-colors duration-200">
                                {guest.Name}
                              </h4>
                              {guest.Email && guest.Email !== "Pending" && (
                                <div className="flex items-center text-[10px] sm:text-xs md:text-sm text-[#6A1F08]/70">
                                  <Mail className="h-2.5 w-2.5 sm:h-3 sm:w-3 md:h-4 md:w-4 mr-1 sm:mr-1.5 text-[#6A1F08] flex-shrink-0" />
                            ...
                            <div className="absolute right-2.5 top-2.5 sm:static sm:right-auto sm:top-auto flex items-center gap-1 sm:gap-1.5">
                              ...
                              <span className="inline-flex items-center justify-center px-2 sm:px-2.5 md:px-3 py-0.5 sm:py-1 bg-[#6A1F08]/10 text-[#6A1F08] rounded-full text-[10px] sm:text-xs md:text-sm font-[family-name:var(--font-crimson)] font-semibold border border-[#6A1F08]/30">
                                {guest.Guest
                          ...
                          {/* Message */}
                          {guest.Message && (
                            <div className="mt-2.5 sm:mt-3 md:mt-4 pt-2.5 sm:pt-3 md:pt-4 border-t border-[#6A1F08]/20">
                              <div className="flex items-start gap-2 sm:gap-2.5 md:gap-3">
                                <MessageSquare className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-[#6A1F08] flex-shrink-0 mt-0.5" />
                                <p className="text-[10px] sm:text-xs md:text-sm text-[#6A1F08]/80 font-[family-name:var(--font-crimson)] leading-relaxed italic flex-1">
                                  "{guest.Message}"
                                </p>
                              </div>
                            </div>
